...
Among the challenges of data adoption perhaps the most highlighted are 'data engineering complexities', 'BD architecture', 'rapid technology change', 'lack of sufficiently skilled data engineers', and 'organization's cultural challenges of becoming data-driven' \cite{AtaeiBigDataEnvirons};\cite{Singh}. This focus of this study is on data engineering complexities and in specific BD architecture.
...
\begin{enumerate}
    \item The Introduction discusses the importance of data but the motivation for this work is not clear. References 1 and 3 point to your prior work - a brief summary of those findings might provide better motivation for this work. Furthermore, reference 3 presents an earlier big data reference architecture that you designed, and the Introduction here should distinguish this contribution from the earlier work.
    \item In §2, the first assumption---"having a sound software architecture is essential to the successful development and maintenance of software systems"---is unsupported and depends on the definition of "success". It isn't clear that this assumption is essential to the paper, and if you choose to retain it, perhaps a weaker assumption than "essential" would suffice.
    \item I found the reasoning in §3.1 to be circular. It seemed that you chose to design a particular type of RA, and this section is a long discussion of how to label the resulting artefact.
    ...
    \item This is a long paper, and could benefit from more extensive signposting to help the reader navigate the material. For example, §3 covers a number of topics, and an up-front roadmap would make this easier to understand.
    ...
    \item The authors state that companies need a new big data strategy and a reference architecture in order to address some challenges and increase the success rate. Nevertheless, throughout the paper, it is no clear what are the current challenges that should be met, what are the business goals that should be addressed, etc. IMHO, this information is key to create the rationale behind the defined reference architecture.
    \item The proposed distributed reference architecture is based on a systematic literature review done by the authors. This analysis is very important. Nevertheless, IMHO the authors should also identify what requirements currently can not be met, what requirements lead to the proposed design, etc. i.e, I think that the authors should include more information about the rationale behind the defined reference architecture.
    ...
    \item  The authors validate the proposed reference architecture by creating a prototype and using ATAM to evaluate them. It would be interesting to know how the prototype has been designed, how the RA helped in the definition of the prototype, etc.
